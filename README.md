...
LuCI 的代理设置支持自动检测、PassWall、OpenClash、SSR Plus、Nikki、daed、HomeProxy 和 MihomoTProxy。启用模块时保存所选代理的配置与运行状态；拨杆可停止代理或恢复快照。自动检测按“正在运行 → 配置启用 → 已安装”选择唯一候选，发现多个候选时报告冲突并要求手动选择。从代理切换到 LED、WiFi 或“不启用”时只停止拨杆管理，不会改变代理当前状态；再次启用代理控制时会重新保存当时的代理状态。
