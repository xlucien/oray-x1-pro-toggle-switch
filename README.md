...
拨杆功能通过一个下拉框在“不启用、LED、WiFi、代理”之间选择，同时最多启用一个。未选择代理时，代理设置区会禁用并降灰；LuCI 后端保存时还会再次校验互斥关系。RESET 多击控制独立运行，不参与该互斥关系，也不受拨杆总开关影响。

页面提供两个操作按钮：“保存”只提交配置，不按当前拨杆位置强制执行动作；“保存并应用”提交后会立即调用当前拨杆位置对应的动作。界面优先使用 LuCI 自带表单和按钮样式，不依赖 CDN 或额外前端框架，并兼容深色主题和手机布局。
